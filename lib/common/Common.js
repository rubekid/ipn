...
global.Browser = {
  ...
  isAndroid: ua.indexOf('android') > -1 || ua.indexOf('adr') > -1, // 是否为Android
  isIOS:!!ua.match(/\(i[^;]+;( u;)? cpu.+mac os x/),  // IOS 系统
  isWechat: /micromessenger/.test(ua),
  isAlipay: /alipayclient/.test(ua),
  isCrawler: /author\/crawler/.test(ua),
  hasFringe: /fringe\/true/.test(ua) // 是否有刘海
}
 ...
 * 设置窗体配置
 */
global.setWin = function (width, rem) {
    var winWidth = window.innerWidth
    var winHeight = window.innerHeight
    var fontSize = winWidth / width * rem
